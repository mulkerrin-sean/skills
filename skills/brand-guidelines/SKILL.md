...
# Deep Research â€” Universal Academic Research Agent Team

Universal deep research tool â€” a domain-agnostic 13-agent team for rigorous academic research on any topic.

**v2.4** adds writing quality improvements to the report compiler:
- **Style Profile consumption** (optional) â€” If a Style Profile is available from academic-paper intake, the report compiler applies it as a soft guide for the Executive Summary and Synthesis sections. Discipline conventions and report objectivity take priority.
- **Writing Quality Check** â€” The report compiler runs a writing quality checklist before finalizing: flags AI-typical overused terms, checks sentence/paragraph length variation, removes throat-clearing openers. See `academic-paper/references/writing_quality_check.md`.

> **Routing discipline (v3.9.2):** see `.claude/CLAUDE.md` "Routing Discipline (v3.9.2)" + `shared/references/intent_clarification_protocol.md` for cross-skill routing rules. This skill assumes routing has already settled â€” ambiguous cross-phase materials should have been clarified upstream.
...
Help me clarify my research direction. I am interested in higher education quality assurance but not yet sure what to study.
```

**Execution:**
1. Scoping â€” Research question + methodology blueprint
2. Investigation â€” Systematic literature search + source verification
3. Analysis â€” Cross-source synthesis + bias check
4. Composition â€” Full APA 7.0 report
5. Review â€” Editorial + ethics + vulnerability scan
6. Revision â€” Final polished report
...
**Default rule**: When intent is ambiguous between `socratic` and `full`, **prefer `socratic`** â€” it is safer to guide first than to produce an unwanted report. The user can always switch to `full` later.

**Example triggers** (illustrative, not exhaustive):
"guide my research", "help me think through", "help me clarify my research direction", or equivalent wording.
...
| Reviewing a paper through structured review | `academic-paper-reviewer` |
...
| Need a quick brief | `quick` | fidelity |
...
Not sure? Start with `socratic` â€” it will help you figure out what you need.
...
| 4 | `source_verification_agent` | Fact-checking, source grading, predatory journal detection, conflict-of-interest flagging | Phase 2 |
| 5 | `synthesis_agent` | Cross-source integration, contradiction resolution, thematic synthesis, gap analysis | Phase 3 |
| 6 | `report_compiler_agent` | Drafts complete APA 7.0 report: Title, Abstract, Introduction, Method, Findings, Discussion, References | Phase 4, 6 |
| 7 | `editor_in_chief_agent` | Q1 journal editorial review: originality, rigor, evidence sufficiency, verdict | Phase 5 |
...
| 10 | `socratic_mentor_agent` | Q1 journal editor persona; guides research thinking through Socratic questioning across 5 layers | Socratic Mode, Layers 1-5 |
| 11 | `risk_of_bias_agent` | Assesses risk of bias using RoB 2 and ROBINS-I; traffic-light visualization | Systematic Review, Phase 2 |
...
**Mode A â€” orchestrator-driven (default):** `pipeline_orchestrator_agent` in the `academic-pipeline` skill runs all phases end-to-end with state tracking via Material Passport.

**Mode B â€” phase-by-phase (cross-session resume):** User invokes one agent per phase across sessions for long-running projects. Common pattern via `ARS_PASSPORT_RESET=1` + `resume_from_passport=<hash>`; see `academic-pipeline/references/passport_as_reset_boundary.md`.

In Mode B, **single-phase agents** stay strictly within their assigned phase for writes. Reads from upstream phases are allowed. Multi-phase agents, including `devils_advocate_agent` and `report_compiler_agent`, do exactly the work specified by the caller's invocation for that phase â€” no extension to other phases in the same call.
...
Setting `ARS_SOCRATIC_READING_PROBE=1` enables a one-time honesty probe during **goal-oriented** Socratic sessions. When the user cites a specific paper, the Mentor asks them to paraphrase one passage. Decline is logged without penalty. Default OFF. See `agents/socratic_mentor_agent.md` Â§"Optional Reading Probe Layer".
...
> **v3.4.0 compliance:** `systematic-review` mode triggers `compliance_agent` at Stage 2.5 for Methods items and Stage 4.5 for remaining items + RAISE 8-role matrix. PRISMA-trAIce Mandatory failures block the pipeline. See `shared/compliance_checkpoint_protocol.md`.
...
|------|---------------|--------|------------|
| `full` (default) | All 9 core agents, excluding socratic_mentor, RoB, meta-analysis | Full APA 7.0 report | 3,000-8,000 |
...
| Insufficient literature | bibliography_agent finds fewer than 5 sources | Expand search strategy and alternative keywords |
...
| Socratic non-convergence | More than 10 rounds without convergence | Suggest switching to full mode |
| User abandons mid-process | Explicitly states they do not want to continue | Save progress, provide re-entry path |
| Only non-English literature available | English search returns empty | Expand search to regional and multilingual databases |
...
## Handoff Protocol: deep-academic-research â†’ academic-paper
...
5. **INSIGHT Collection and Research Plan Summary**, if socratic mode was used
...
| `references/systematic_review_toolkit.md` | Cochrane v6.4, PRISMA 2020, RoB 2, ROBINS-I, IÂ² guide, GRADE, protocol registration | risk_of_bias, meta_analysis, bibliography, report_compiler |
...
| `references/argumentation_reasoning_framework.md` | Cognitive framework for evaluating argument strength: Toulmin model, causal reasoning, inference to best explanation, epistemic status classification | synthesis, devils_advocate, source_verification, socratic_mentor, research_architect |
...
| `references/changelog.md` | Full version history | â€” |
...
Follows the user's language. Academic terminology is kept in English. Socratic mode uses natural conversational style.
...
| 4 | **IRON RULE: Treating "difficult to verify" as acceptable** | Marking a reference as "uncertain" instead of FAIL | Gray zone = FAIL. If you cannot confirm it exists, it does not go in the report |
...
| 6 | **Shallow Socratic mode** | Giving answers disguised as questions | Ask genuine questions that expose assumptions; never lead to predetermined conclusions |
| 7 | **Source tier inflation** | Treating a blog post as equivalent to a peer-reviewed journal | Apply evidence hierarchy strictly: Tier 1 peer-reviewed > Tier 2 preprint > Tier 3 gray literature |

## Quality Standards

1. **IRON RULE**: **Every claim must have a citation** â€” no unsupported assertions.
2. **Evidence hierarchy** â€” meta-analyses > RCTs > cohort studies > case reports > expert opinion.
3. **Contradiction disclosure** â€” if sources disagree, report both sides with evidence quality comparison.
4. **Limitation transparency** â€” every report must have an explicit limitations section.
5. **AI disclosure** â€” all reports include a statement that AI-assisted research tools were used.
6. **Reproducibility** â€” search strategies, inclusion criteria, and analytical methods must be documented for replication.
7. **Socratic integrity** â€” in socratic mode, never give direct answers; always guide through questions.

## Cross-Agent Quality Alignment

Unified definitions across all agents. **IRON RULE**: **CRITICAL severity** = issue that would invalidate a core conclusion or constitute academic misconduct. Requires immediate resolution.
...
```text
deep-academic-research + tw-hei-intelligence      -> Evidence-based HEI policy research
deep-academic-research + report-to-website        -> Interactive research report
deep-academic-research + podcast-script-generator -> Research podcast
deep-academic-research + academic-paper           -> Full research-to-publication pipeline
deep-academic-research (socratic) + academic-paper (plan) -> Guided research + paper planning
deep-academic-research (systematic-review) + academic-paper -> PRISMA systematic review paper
```
...
> See `references/changelog.md` for full version history.- Maintains color fidelity across different systems
