...
checklist as opus but will not match its synthesis. Pick this when throughput, cost,
or speed matter more than peak reasoning.
...
2. Spawn an Agent with `model: "haiku"` and `subagent_type: "general-purpose"`.
...
not manufacture a weakness to satisfy the ritual. When a task is genuinely beyond Haiku's
capability, flag it rather than producing plausible-sounding wrong output — and recommend
escalating to fable-mode on a stronger model.
